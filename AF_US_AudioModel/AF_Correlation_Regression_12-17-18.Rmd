---
title: "AF_Correlation_Regression_Modeling"
author: "MLee and cmosle1"
date: "11/7/2018"
...
#q0034- Do your ever get noises such as ringing or buzzing in your ears or head (tinnitus) that last         more than 5 ninutes? (If yes) Do you get these noises some of the time, most of the time, or         all of the time? 1=No; 2=Yes, some of the time; 3= Yes, most or all of the time
...
AUS_data$state_id <- 'NSW'
AUS_data$city_id <- 'Sydney'
US_data$country_id <- 'US'
US_data$state_id <- 'WA'
US_data$city_id <- 'Seattle'
UK_data$country_id <- 'UK'
UK_data$region_id <- 'England'
UK_data$state_id <- 'London'
UK_data$city_id <- 'City of London'

CAN_data$country_id <- 'CAN'
CAN_data$state_id <- 'Ontario'
CAN_data$city_id <- 'Toronto'
...
#boot_out <- replicate(1,model_boot_split(US_data))
...
Exploratory analysis: Table analysis of responses
```{R}
...
Train and test model on US vs other countries
```{r}
...
Train and test model using bootstrapping on Global data set
...
UK_state_RN_data <- read.spss("C:/Users/cwbishop/Documents/dropbox/Dropbox/Kelly and Michael/England eHearing Study - Online Panel Survey.sav", to.data.frame=TRUE,use.value.labels=FALSE)
...
London_RN_data <- read.spss("C:/Users/cwbishop/Documents/dropbox/Dropbox/Kelly and Michael/Greater London eHearing Study - Online Panel Survey.sav", to.data.frame=TRUE,use.value.labels=FALSE)
...
# Survey was not completed if Q46 had no response
#all_RN_data<- all_RN_data[complete.cases(all_RN_data$q0046),]

#Merge global RN and HHT datasets

ehearing_data <- rbind.fill(all_RN_data,global_data)
ehearing_data <- ehearing_data[complete.cases(ehearing_data$q0046),]
#Remove panel and study ID information. During the import process, these two columns get spread over about 100 columns

ehearing_data <- ehearing_data[,-c(1475:1570,1600)]
###Need to output dataset
...
```{r}
ehearing_data$hcat_impute <- predict(boot_ormod_all,ehearing_data)

write.csv(ehearing_data,paste("ehearing ",Sys.Date(),".csv",sep=""),row.names=F)
...
# grid.table(round(prop.table(df,2)*100,3))
# 
# #Explore response drop off
# sum(table(CAN_RN_data$q0002))
# sum(table(CAN_RN_data$q0003))
# sum(table(CAN_RN_data$q0004))
# sum(table(CAN_RN_data$q0005))
# sum(table(CAN_RN_data$q0006))
# sum(table(CAN_RN_data$q0007))
# sum(table(CAN_RN_data$q0008))
# sum(table(CAN_RN_data$q0009_0001))
# sum(table(CAN_RN_data$q0046))
# 
# sum(table(CAN_state_RN_data$q0002))
# sum(table(CAN_state_RN_data$q0003))
# sum(table(CAN_state_RN_data$q0004))
# sum(table(CAN_state_RN_data$q0005))
# sum(table(CAN_state_RN_data$q0006))
# sum(table(CAN_state_RN_data$q0007))
# sum(table(CAN_state_RN_data$q0008))
# sum(table(CAN_state_RN_data$q0009_0001))
# sum(table(CAN_state_RN_data$q0046))
# 
# sum(table(UK_RN_data$q0002))
# sum(table(UK_RN_data$q0003))
# sum(table(UK_RN_data$q0004))
# sum(table(UK_RN_data$q0005))
# sum(table(UK_RN_data$q0006))
# sum(table(UK_RN_data$q0007))
# sum(table(UK_RN_data$q0008))
# sum(table(UK_RN_data$q0009_0001))
# sum(table(UK_RN_data$q0046))
# 
# CAN_RN_data$StartDate[is.na(CAN_RN_data$q0008)]
```