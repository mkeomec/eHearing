...
```{r}
 ## Split global data and use global model to predict HCAT.
    ...
    boot_ormod_data <- global_data[,c('hcat','q0002','q0003','q0010','q0032','q0042','q0046')]
    boot_ormod_data <-  boot_ormod_data[complete.cases(boot_ormod_data),]
    ...
    boot_ormod_all <- polr(hcat~factor(q0002)+factor(q0003)+factor(q0010)+factor(q0032)+ factor(q0042)+factor(q0046), data = mangroup2, Hess=TRUE)
...
test_US <- as.data.frame(attr(US_state_RN_data,'variable.labels'))
test_US <- as.data.frame(attr(US_RN_data,'variable.labels'))
